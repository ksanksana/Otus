...
```
Настраиваю статичекое и динамическое кеширование.
...
Клиентское кеширование уже было настроено по-умолчанию, менять не стала.

SSL у меня не настроен, так что HTTP/2 и HTTP/3 включать пока смысла нет.
...
Тест после всех оптимизаций почему-то показал худший результат, хотя в браузере видно что и картинки отдаются avif, и сжатие работает.
